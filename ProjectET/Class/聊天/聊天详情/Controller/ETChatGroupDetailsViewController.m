//
//  ETChatGroupDetailsViewController.m
//  ProjectET
//
...
@interface ETChatGroupDetailsViewController ()
...
@implementation ETChatGroupDetailsViewController
...
- (void)socketLine{
    [[ETSocketHelper sharedSocketHelper] socketRequest:@"http://47.244.50.67:2569" AndKey:@"refresh" success:^(id  _Nonnull responseObject) {
        ...
        if ([model.towho isEqualToString:self.qcode]) {
            model.photo = model.fromphoto;
...
- (void)getTochating{
    ...
    [HTTPTool requestDotNetWithURLString:@"togroupchating" parameters:@{@"fromwho":model.address,@"togroup":self.qcode,@"text":self.tf_msg.text} type:kPOST success:^(id responseObject) {
...
- (void)getChatingorder{
    ...
    [HTTPTool requestDotNetWithURLString:@"chatinggrouporder" parameters:@{@"fromwho":model.address,@"qcode":self.qcode,@"page":[NSString stringWithFormat:@"%ld",(long)self.currentPage]} type:kPOST success:^(id responseObject) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([chatModel.fromwho isEqualToString:model.address]) {
        ...
        [cell.iconImageView sd_setImageWithURL:[NSURL URLWithString:chatModel.photo]];
        return cell;
...
- (void)rightBtnAction{
    ETGroupChatInformationViewController *vc = [ETGroupChatInformationViewController new];
    vc.qcode = self.qcode;
