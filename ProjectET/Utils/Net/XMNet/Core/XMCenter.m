...
- (void)xm_sendRequest:(XMRequest *)request {
    ...
    if (self.consoleLog) {
        if (request.requestType == kXMRequestDownload) {
            NSLog(@"\n============ [XMRequest Info] ============\nrequest download url: %@\nrequest methodType: %@ \nrequest save path: %@ \nrequest headers: \n%@ \nrequest parameters: \n%@ \n==========================================\n", request.url, [self xm_requestHttpMethodType:request.httpMethod],request.downloadSavePath, request.headers, request.parameters);
        } else {
            NSLog(@"\n============ [XMRequest Info] ============\nrequest url: %@ \nrequest methodType: %@ \nrequest headers: \n%@ \nrequest parameters: \n%@ \n==========================================\n", request.url, [self xm_requestHttpMethodType:request.httpMethod],request.headers, request.parameters);
        }
    }
...
- (void)xm_successWithResponse:(id)responseObject forRequest:(XMRequest *)request {
    ...
    if (self.consoleLog) {
        if (request.requestType == kXMRequestDownload) {
            NSLog(@"\n============ [XMResponse Data] ===========\nrequest download url: %@ \nrequest methodType: %@ \nresponse data: %@\n==========================================\n", request.url, [self xm_requestHttpMethodType:request.httpMethod],
//                  responseObject
                  @""
                  );
        } else {
...
- (void)xm_failureWithError:(NSError *)error forRequest:(XMRequest *)request {
    ...
    if (self.consoleLog) {
        NSLog(@"\n=========== [XMResponse Error] ===========\nrequest url: %@ \nrequest methodType: %@ \nerror info: \n%@\n==========================================\n", request.url, [self xm_requestHttpMethodType:request.httpMethod],error);
    }
...
- (NSString *)xm_requestHttpMethodType:(XMHTTPMethodType)type {
    switch (type) {
